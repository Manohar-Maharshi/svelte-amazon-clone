...
<nav class="h-[60px] w-full flex items-center bg-[#131921] !text-gray-100">
		<div class="w-full flex items-center justify-between space-x-10 mx-auto px-4">
			...
			<div class="md:flex hidden  cursor-default leading-tight text-sm flex items-start flex-col">
			...
			<div class="flex-1 w-full md:flex hidden ">
				<form class="w-full flex items-center">
					...
					<button class="rounded-r bg-[#e47911] py-2.5 w-10 flex items-center justify-center">
			...
			<div class="flex items-center space-x-5">
				<div class="leading-tight text-sm flex items-start flex-col">
					{#if $user}
						<p class="text-xs opacity-80">{ $user ? `Hello,${$user.displayName || $user.email}` : 'Hello,Guest' }</p>
						<p on:click={loginOut} class="cursor-pointer font-semibold flex items-center flex items-center space-x-1">
					...
					{:else}
						<p class="text-xs opacity-80">{ $user ? `Hello,${$user.displayName || $user.email}` : 'Hello,Guest' }</p>
						<p on:click={loginWithGoogle} class="cursor-pointer font-semibold flex items-center flex items-center space-x-1">
							<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-3 w-3" viewBox="0 0 16 16">
							  <path d="M15.545 6.558a9.42 9.42 0 0 1 .139 1.626c0 2.434-.87 4.492-2.384 5.885h.002C11.978 15.292 10.158 16 8 16A8 8 0 1 1 8 0a7.689 7.689 0 0 1 5.352 2.082l-2.284 2.284A4.347 4.347 0 0 0 8 3.166c-2.087 0-3.86 1.408-4.492 3.304a4.792 4.792 0 0 0 0 3.063h.003c.635 1.893 2.405 3.301 4.492 3.301 1.078 0 2.004-.276 2.722-.764h-.003a3.702 3.702 0 0 0 1.599-2.431H8v-3.08h7.545z"/>
							</svg>
							<span>Sign In</span>
					...
					<NavLink to="wish-list">
						<div title="Favorite Products in One Place">
							<span class="font-bold flex items-center space-x-1 font-bold">
								<svg xmlns="http://www.w3.org/2000/svg" class=" h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
								</svg>
								<span class="md:flex hidden ">WishList</span>
							</span>
						</div>
					</NavLink>
				<NavLink to="cart">
					<div title="Your Cart items" class="flex items-center space-x-1">
						<div>
							<svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" >
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
							</svg>
						</div>
						<p class="font-semibold leading-none">{cartItemsLength || 0}</p>
					</div>
				</NavLink>
			</div>
		</div>
</nav>
<form class="md:hidden flex items-center m-2">
	<input type="text" class="rounded-l focus:outline-none rounded-none w-full px-2 py-2 border border-gray-900 text-[#131921]">
	<button class="rounded-r bg-[#e47911] py-3 w-[3rem] flex items-center justify-center">
		<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#131921]" viewBox="0 0 20 20" fill="currentColor">
		  <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd" />
		</svg>
	</button>
